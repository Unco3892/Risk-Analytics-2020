...
Expected shortfall or conditional or sometimes called conditional value-at-risk , is about 'how bad can things get?'. More concrectly, one could compute the Value-at-Risk at a level of  `r cf_news`, and compute the expected shortfall which indicates the average value of sales when we enter the etremes of the distribution, hence when the sales are extremely high which, as we saw in the beginning of this analysis, is likely to lead to a stockout.
...
critical_fractile_poisson <- -((log(1-cf_news))/sales_poisson$estimate)
...
```
After having fitted a Poisson to our data, we obtain μ = `r mu`. This allows us to compute the critical fractile, which is given by the following formula : q = -ln(1- `r cf_news`)/`r mu`. According to this model, the estimated critical fractile is `r critical_fractile_poisson`
...
VAR <- mu + gpd_u170$estimate[1]/gpd_u170$estimate[2] * (((1-cf_news)/p.sales)^-gpd_u170$estimate[2]-1)
