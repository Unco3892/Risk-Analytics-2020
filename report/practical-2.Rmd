...
Thresholds of 150 and 170 seem interesting, we can test for all three.
...
ilia: comment to be modified/removed?
...
# This method or the next? Which paramters are right?
prop.test((nrow(sales_test)-sales_violations),nrow(sales_test), cf_news)

binom.test(sales_violations,nrow(sales_test), p=1-cf_news) # not sure if the conf.level or the p
...
In order to set the `u`, we can also do use the 90th of the data for the threshold.
...
# higher than the quantile, not possible!

# summing up violations
sales_violations <- sum(risk_values)

# histogram of all the p hats and you check if the test stat is the center of
# of distribution

# Naturally our critical fractile is the alpha here, right?
# prop.test((nrow(sales_test)-sales_violations),nrow(sales_test), cf_news)

# bootstrapping, that we have a distibution for it
