...
waiting %>%
  ...
  geom_boxplot() +
  coord_flip() +
  labs (title = "Average waiting time per day",
        x = "Day",
        y = "Time [seconds]")
```
...
Naturally, having a higher threhsold the number of observations is lower. Looking at the results, we can conclude that we should select the threshold `r u1` since it allows having a better pp-plot and qq-plot.
...
return_level_maxima <- fit_gev_maxima_weekly$estimate[1] + (fit_gev_maxima_weekly$estimate[2]/fit_gev_maxima_weekly$estimate[3])*(((-log(1-1/365))^-fit_gev_maxima_weekly$estimate[3])-1)

return_level <- return_level_maxima[[1]]
return_level
```

After one year, we expect the Average waiting time in seconds to be of `r return_level` seconds. 
We are going to use this data in order to compute first the level, then the Value-at-Risk which is going to be providing the confidence line. 
...
# confidence_line <- u1 + fit_pot_u1$estimate[1]/fit_pot_u1$estimate[2] * (((1-p)/p.survivors)^-fit_pot_u1$estimate[2]-1)
# 
# confidence_line[[1]]

alpha <- 1 - 1/return_level
VAR <- u1 + fit_pot_u1$estimate[1]/fit_pot_u1$estimate[2]*(((1-alpha)/p.survivors)^-fit_pot_u1$estimate[2] -1)

```

We expect the confidence line to be at `r VAR`.
