...
=======
  full_join(max_per_month, by = c("Average.Wait.Seconds" = "Max.Seconds")) %>% 
>>>>>>> da19925a02f8194da6d2d03a65c631c9ff6bc8ed
>>>>>>> Stashed changes
...
Looking at the data aggregation per month, week and year we conclude that the best way to break the blocks is per month. 
...
mrl.plot(waiting$Average.Wait.Seconds,umin = min(waiting$Average.Wait.Seconds), umax = max(waiting$Average.Wait.Seconds))
mrlplot(log(waiting$Average.Wait.Seconds),conf = 0.95, main="Mean Residual Life Plot")
```

We have computed two Mean Residual Rife Plots. In the second, we have transformed the data in log, allowing to have an estimated mean excess function almost perfectly linear. This result shows that it may be better to use such transformation when working with the POT. 
Moreover, the pot allows us to have to define the threshold u, which in this case we would set to 4.5
...
waiting %>% 
  ggplot(aes(x=Date, y= log(waiting$Average.Wait.Seconds))) +
  geom_point()
```
...
Looking at the output, we can see that $\xi=$`r fit_gev$par[3]`, $\sigma=$`r fit_gev$par[2]`, and $\mu$`r fit_gev$par[1]`. The $\xi=$ is negative and indicates that this may be a Fréchet distribution, which means that the distribution has an upper bound and cannot go until infinity, however, it is very close to zero therefore we cannot be certain of it's shape.
...
- Return period from the extRemes goes up to 1,000 years, why? It is not even the block size.
- How to the return levels?
...
# 1 year return level
waiting %>% 
  group_by(year(ymd(waiting$Date))) %>% 
  count()
```

Having 250 observation for the first year, we consider 1 year = 250 days. 
Hence, our control limit will be the value that we expect to exceed every 250 days, which we can compute as follows:

```{r}
return.level(waiting$Average.Wait.Seconds,
    do.ci = FALSE, qcov = NULL, qcov.base = NULL)


var <-var(waiting$Average.Wait.Seconds, y = NULL)
fevd(var)

rgev(447, loc=0, scale=1, shape=1)
