...
``` {r}
library(ismev)
fit_gev_maxima_evir<-gev.fit(maxima$Average.Wait.Seconds)
gev.diag(fit_gev_maxima_evir)
...
# Use ismev with gpd.fit() and then gdp.diag to do it
gpd.fit()
gpd.diag()
```
```{r}
# Original code needs changing
