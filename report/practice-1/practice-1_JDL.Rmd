...
waiting$col <- cut(waiting$Average.Wait.Seconds,
               breaks = c(-Inf, u, Inf),
               labels = c("<=u", ">u")) 
...
waiting %>%
  ...
  geom_point(aes(Date, Average.Wait.Seconds, color = col)) +
...
fit_gev_maxima <- fgev(maxima_monthly$Average.Wait.Seconds)
...
```{r}
n.survivors <- waiting %>%
  filter(Average.Wait.Seconds > u) %>%
  summarize(n = n()) %>%
  pull(n)

p.survivors <- n.survivors/length(waiting$Average.Wait.Seconds)

VaR <- u + pot_800$estimate[1]/pot_800$estimate[2] * (((1-p)/p.survivors)^-pot_800$estimate[2]-1)
```
